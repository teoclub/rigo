...
import type { AttachmentStore } from '@teoclub/harness-attachment'
import { admitEncodedImages } from '@teoclub/harness-attachment'
import type { ImageAttachmentRef, SaveImageAttachment } from '@teoclub/harness-attachment/types'
...
describe('admitEncodedImages', () => {
  ...
  it('delegates an empty batch unchanged', async () => {
    const { store, mocks } = storeOf()
    await expect(Promise.resolve(admitEncodedImages(store, []))).resolves.toEqual([])
  ...
  it('rejects non-canonical and empty base64 payloads before any store call', async () => {
    ...
    for (const data of ['', 'AAA', '!!!!']) {
      await expect(Promise.resolve(admitEncodedImages(store, [{ mediaType: 'image/png', data }])))
  ...
  it('propagates the store batch rejection unchanged', async () => {
    ...
    await expect(Promise.resolve(admitEncodedImages(store, [{ mediaType: 'image/png', data: PNG }]))).rejects.toBe(refused)
  })
})

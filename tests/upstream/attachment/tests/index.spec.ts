...
} from '@teoclub/harness-attachment'
...
describe('AttachmentStore.saveImages', () => {
  ...
  it('rejects count, aggregate bytes, and deployment media types before validation', async () => {
    ...
    await expect(Promise.resolve(store.saveImages([image(1), image(2), image(3)])))
      .rejects.toMatchObject({ code: 'TOO_MANY_IMAGES' })
    await expect(Promise.resolve(store.saveImages([
    ...
    await expect(Promise.resolve(store.saveImages([image(1, 'image/jpeg')])))
  ...
  it('starts no writes when any member fails validation', async () => {
    ...
    await expect(Promise.resolve(store.saveImages([image(1), image(2)])))
...
describe('AttachmentStore.readImageRequest', () => {
  it('reports unsupported request projection while preserving cancellation', async () => {
    ...
    await expect(Promise.resolve(store.readImageRequest(ref, { maxPixels: 1, maxBytes: 1 })))

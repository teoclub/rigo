...
describe('HMR exact config paths', () => {
  it.skipIf(RIGO_REQUIRES_NODE_INTERNALS)('observes module changes when its watch base is a filesystem alias', { timeout: 30_000 }, async () => {
  ...
  it('collapses filesystem aliases before registering an exact watch', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(ctx.hmr.registerConfig(join(await realpath(target), 'plugins.yml'), () => {})))

const RIGO_REQUIRES_NODE_INTERNALS = typeof Bun !== 'undefined'
...
import SystemPrompt, { renderPrompt } from '@teoclub/harness-system-prompt'
import {
...
describe('installFailLoud', () => {
  ...
  it('does not report an activation rejection shared by entries in the boot audit', async () => {
    ...
    await expect(Promise.resolve(audit)).rejects.toThrow('assembled activation failure')
    proc.handlers[0]!(error)
    expect(proc.exits).toEqual([1])
  })
...
describe('assertEntriesActivated', () => {
  ...
  it('passes active entries and ignores disabled entries', async () => {
    ...
    await expect(Promise.resolve(assertEntriesActivated(ctxWith([
...
describe('boot', () => {
  ...
  it.skipIf(RIGO_REQUIRES_NODE_INTERNALS)('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    const dir = tmp()
    const harness = tmp()
  ...
  it('disposes partial host setup and labels non-Error preparation failures', async () => {
    ...
    await expect(Promise.resolve(task)).rejects.toMatchObject({
  ...
  it('falls back to the deepest cause message when its stack was erased', async () => {
    ...
    await expect(Promise.resolve(boot(NAME, join(dir, 'cordis.yml'), undefined, () => {

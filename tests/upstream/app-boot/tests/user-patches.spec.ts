...
describe('boot with user patches', () => {
  ...
  it('fails loud when the exact watcher lacks HMR or a root Include', async () => {
    ...
    await expect(Promise.resolve(watchUserPatches(withoutHmr, { binName: NAME, filename: join(tmp(), PROFILE_PATCH_FILENAME) }))).rejects.toThrow('requires the Cordis HMR service')
    ...
    await expect(Promise.resolve(watchUserPatches(withoutInclude, { binName: NAME, filename: join(tmp(), PROFILE_PATCH_FILENAME) }))).rejects.toThrow('requires the root Include entry')
  ...
  it('propagates registration failures other than mid-teardown', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(watchUserPatches(ctx, { binName: NAME, filename }))).rejects.toThrow('already registered')
      await dispose()
    } finally {
      await ctx.fiber.dispose()
    }

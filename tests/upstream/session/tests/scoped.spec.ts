...
import { createScope, scopeOf } from '@teoclub/harness-scope'
import type { Scope, ScopeKey } from '@teoclub/harness-scope'
...
describe('sessions.flush()', () => {
  it('allows an ordinary flush with no listeners', async () => {
    ...
    await expect(Promise.resolve(ctx.sessions.flush(session))).resolves.toBe(false)
  })
  ...
  it('propagates a rejecting flush listener (the caller owns the failure policy)', async () => {
    ...
    const session = ctx.sessions.create()
    await expect(Promise.resolve(ctx.sessions.flush(session))).rejects.toThrow('disk full')
  })
  ...
  it('waits for slower flush listeners before reporting another listener failure', async () => {
    ...
    await expect(Promise.resolve(flushing)).rejects.toThrow('disk full')
  ...
  it('rejects a never-entered session instead of inventing a carrier', async () => {
    ...
    await expect(Promise.resolve(ctx.sessions.flush(prepared))).rejects.toThrow(/not live/)
  ...
  it('clears a detached carrier and rejects stale flushes', async () => {
    ...
    detach()
    await expect(Promise.resolve(ctx.sessions.flush(session))).rejects.toThrow(/not live/)

...
import { createUserMessage, freezeMessage } from '@teoclub/harness-llm'
import { Session, SessionId, type UserMessage } from '@teoclub/harness-session'
import AgentRegistry, {
  agentEvents,
  Inbox,
} from '@teoclub/harness-agent'
import TypertRegistry from '@teoclub/harness-typert-registry'
...
describe('AgentRegistry factory seam', () => {
  ...
  it('requires a factory and delegates through the calling context', async () => {
    const ctx = new Context()
    await ctx.plugin(AgentRegistry)
    await expect(Promise.resolve(ctx.agents.create({ sessionId: SessionId('s') }))).rejects.toThrow(/no agent factory/)
  ...
  it('rejects a second factory and clears the slot with its owner (HMR)', async () => {
    ...
    await expect(Promise.resolve(ctx.agents.create({ sessionId: SessionId('before-s') }))).resolves.toBeDefined()
    await owner.dispose()
    await expect(Promise.resolve(ctx.agents.create({ sessionId: SessionId('after-s') }))).rejects.toThrow(/no agent factory/)
  })

...
import type { Agent } from '@teoclub/harness-agent'
import { CallId } from '@teoclub/harness-llm'
import { carrierKeyOf, createScope } from '@teoclub/harness-scope'
import type { Scope } from '@teoclub/harness-scope'
...
import ApprovalService, { ApprovalOutcome, ApprovalRequest, effectiveApprovalPolicy, setApprovalPolicy } from '@teoclub/harness-user-approval'
...
describe('ApprovalService.request', () => {
  it('throws before appending anything when no turn has ever opened (idle ask)', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(requestOf(agent)))).rejects.toThrow(/outside an open turn/)
  ...
  it('borrows the exact readonly request for scoped dispatch and audit', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(request))).resolves.toBe('allowed-once')
  ...
  it('contains an approval/asked observer throw after append and still completes the pair', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(requestOf(agent)))).resolves.toBe('allowed-once')
  ...
  it('contains an approval/decided observer throw after append and still resolves', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(requestOf(agent)))).resolves.toBe('rejected')
  ...
  it('propagates an append failure that prevented audit log growth', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(requestOf(agent)))).rejects.toBe(failure)
  })
  ...
  it('lets a non-owning listener delegate via next() down to the fail-closed default', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(requestOf(agent)))).resolves.toBe('unavailable')
  })
  ...
  it('dispatches to global and matching agent-scoped listeners, never a foreign scope', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(requestOf(agentA)))).resolves.toBe('unavailable')
    await expect(Promise.resolve(ctx.approval.request(requestOf(agentB)))).resolves.toBe('unavailable')
  ...
  it('resolves cancelled when the signal aborts mid-question and discards the late answer', async () => {
    ...
    controller.abort()
    await expect(Promise.resolve(pending)).resolves.toBe('cancelled')
  ...
  it('resolves the answer when the signal never aborts', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request(requestOf(agent, { signal: controller.signal })))).resolves.toBe('rejected')
  })
...
describe('approval policy (the approval/policy fold)', () => {
  ...
  it('defaults a schema-less construction to ask (the ?? narrows the optional TYPE)', async () => {
    ...
    ctx.on('approval/request', () => Promise.resolve<ApprovalOutcome>('allowed-once'))
    await expect(Promise.resolve(service.request({ agent, toolName: 'echo' }))).resolves.toBe('allowed-once')
  })
  ...
  it('contains an answerer that throws SYNCHRONOUSLY as unavailable', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request({ agent, toolName: 'echo' }))).resolves.toBe('unavailable')
  })
  ...
  it('a never config rejects deterministically without consulting any answerer', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request({ agent, toolName: 'bash' }))).resolves.toBe('rejected')
  ...
  it('a session override outranks the configured default, in both directions', async () => {
    ...
    await expect(Promise.resolve(ctx.approval.request({ agent, toolName: 'bash' }))).resolves.toBe('allowed-once')
    setApprovalPolicy(session, 'never')
    await expect(Promise.resolve(ctx.approval.request({ agent, toolName: 'bash' }))).resolves.toBe('rejected')
  })

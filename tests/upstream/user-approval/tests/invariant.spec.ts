...
import { ApprovalRequestId } from '@teoclub/harness-user-approval'
import * as ApprovalInvariant from '@teoclub/harness-user-approval/invariant'
...
describe('approval invariants', () => {
  ...
  it('rejects an unenclosed audit event when replaying an existing session', async () => {
    ...
    await ctx.plugin(InvariantRegistry)
    await expect(Promise.resolve(ctx.plugin(ApprovalInvariant).then(() => undefined))).rejects.toThrow(/outside any open turn/)
  })

...
import { Context, type Fiber } from '@teoclub/cordis'
import { scopeTarget } from '@teoclub/harness-scope'
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId } from '@teoclub/harness-session'
  ...
  describe(`PersistenceCoordinator orchestration: ${name}`, () => {
    ...
    it('rejects crash-repair load while a live session owns the persisted prefix', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessionPersistence.load(session.id)))
    ...
    it('rechecks live ownership after a cold load enters the per-id chain', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(loading)).rejects.toThrow(/live turn is open/)
    ...
    it('does not load an unmaterialized empty live session', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessionPersistence.load(session.id))).rejects.toThrow(/not found/)
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
    ...
    it('rejects malformed persisted message events before returning them', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessionPersistence.inspect(id)))
        ...
        for (const malformed of malformedLegacy) {
          ...
          await expect(Promise.resolve(ctx.sessionPersistence.inspect(malformedId))).rejects.toThrow(malformed.message)
          await expect(Promise.resolve(ctx.sessionPersistence.readFrom(malformedId, 0))).rejects.toThrow(malformed.message)
        }
        ...
        for (const type of ['tool/result'] as const) {
          ...
          await expect(Promise.resolve(ctx.sessionPersistence.inspect(malformedId)))
        ...
        await expect(Promise.resolve(ctx.sessionPersistence.inspect(pluginId)))
          .resolves.toMatchObject({ events: [{ type: 'plugin/test', data: null, ignorable: true }] })
        await expect(Promise.resolve(ctx.sessionPersistence.readFrom(pluginId, 0)))
          .resolves.toMatchObject({ events: [{ type: 'plugin/test', data: null, ignorable: true }] })
        ...
        for (const type of ['user/message', 'assistant/message'] as const) {
          ...
          await expect(Promise.resolve(ctx.sessionPersistence.readFrom(missingContentId, 0)))
            .rejects.toThrow('lacks an identified message')
        }
      } finally {
        await fiber.dispose()
    ...
    it('HMR: reloading the backend adopts a still-live, already-materialized session', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessions.flush(session))).resolves.toBe(true)
    ...
    it('a NEW live session colliding on a persisted id is rejected, not silently adopted', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(second.ctx.sessions.flush(s2)))
    ...
    it('an abandoned lazy session (never materialized) releases its id for reuse', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessions.flush(reuse))).resolves.toBe(true)
    ...
    it('session disposal drains buffered events before retiring ownership', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessions.flush(reuse))).rejects.toThrow(/persisted log|id collision/)
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
    ...
    it('a live session claims cursor-0 ownerless state created via the public API and persists its seed', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessions.flush(live))).resolves.toBe(true)
    ...
    it('a fresh session reusing a previously-loaded id is rejected (ownerless guard)', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessions.flush(fresh)))
    ...
    it('a live session at a DIFFERENT cwd cannot claim cursor-0 ownerless state (cwd scope)', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(ctx.sessions.flush(live))).rejects.toThrow(/different cwd|id collision/)
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
    ...
    it('load and inspect reject a missing session', async () => {
      ...
      const { ctx, fiber } = await freshCtx(fix)
      try {
        await expect(Promise.resolve(ctx.sessionPersistence.load(SessionId('nope')))).rejects.toThrow(/not found/)
        await expect(Promise.resolve(ctx.sessionPersistence.inspect(SessionId('nope')))).rejects.toThrow(/not found/)
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
    ...
    it('create rejects a duplicate id (in memory and on a persisted log)', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(first.ctx.sessionPersistence.create(m))).rejects.toThrow(/already exists in this backend/)
        await first.ctx.sessionPersistence.append(m.id, oneTurnLog())
      } finally {
        await first.fiber.dispose()
      }
      ...
      const second = await freshCtx(fix)
      try {
        await expect(Promise.resolve(second.ctx.sessionPersistence.create(meta('dup', WORK))))

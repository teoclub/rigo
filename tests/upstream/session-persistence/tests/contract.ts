...
import { SESSION_FORMAT_VERSION, Session, SessionId, TOOL_NOT_STARTED, TOOL_OUTCOME_UNKNOWN } from '@teoclub/harness-session'
import type { SessionEvent, SessionHeader, SurfaceEventType, SurfaceIntent } from '@teoclub/harness-session'
import { CallId, MessageId, createMessage, freezeMessage } from '@teoclub/harness-llm'
import type { SessionPersistence } from '@teoclub/harness-session-persistence'
  ...
  describe(`SessionPersistence contract: ${name}`, () => {
    ...
    it('rejects a fractional creation timestamp without reserving its session id', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(persistence.create(m)))
    ...
    it('rejects pre-aborted observation reads with the exact cancellation reason', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(persistence.listSnapshots(controller.signal))).resolves.toEqual([])
        controller.abort(reason)

        await expect(Promise.resolve(persistence.list(controller.signal))).rejects.toBe(reason)
        await expect(Promise.resolve(persistence.listSnapshots(controller.signal))).rejects.toBe(reason)
        await expect(Promise.resolve(persistence.inspect(SessionId('cancelled-inspect'), controller.signal)))
          .rejects.toBe(reason)
        await expect(Promise.resolve(persistence.readFrom(SessionId('cancelled-read-from'), 0, controller.signal)))
          .rejects.toBe(reason)
      } finally {
        await dispose()
      }
    ...
    it('readFrom returns exactly the stored suffix from the requested seq, without mutating the log', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(persistence.readFrom(m.id, log.length))).resolves.toMatchObject({ events: [] })
        await expect(Promise.resolve(persistence.readFrom(m.id, log.length + 100))).resolves.toMatchObject({ events: [] })
        ...
        await expect(Promise.resolve(persistence.readFrom(SessionId('absent-read-from'), 0))).rejects.toThrow('not found')
        await expect(Promise.resolve(persistence.readFrom(m.id, -1))).rejects.toThrow('non-negative safe integer')
        await expect(Promise.resolve(persistence.readFrom(m.id, 1.5))).rejects.toThrow('non-negative safe integer')
      } finally {
        await dispose()
      }
    })
    ...
    it('append rejects a batch whose first seq does not match the stored next-seq', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(persistence.append(m.id, restated))).rejects.toThrow()
      } finally {
        await dispose()
      }
    })
    ...
    it('append rejects a mid-batch seq gap', async () => {
      ...
      try {
        ...
        await expect(Promise.resolve(persistence.append(m.id, gapped))).rejects.toThrow()
      } finally {
        await dispose()
      }
    })
    ...
    it('append rejects non-JSON-serializable event data, naming the event type', async () => {
      ...
      try {
        ...
        for (const [i, bad] of badValues.entries()) {
          ...
          await expect(Promise.resolve(persistence.append(mi.id, events))).rejects.toThrow(/losslessly JSON-serializable/)
        }
      } finally {
        await dispose()
      }

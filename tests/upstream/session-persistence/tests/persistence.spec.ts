import '../../../support/vitest-bun-polyfill.ts'
import { describe, expect, it, vi } from 'vitest'
import { Context } from '@teoclub/cordis'
import SessionStore, { Session, SessionId, isJsonValue } from '@teoclub/harness-session'
import type { SessionEvent, SessionHeader } from '@teoclub/harness-session'
import {
...
describe('PersistenceCoordinator bounded writes', () => {
  it('cancels the batching deadline when live initialization rejects', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(ctx.sessions.flush(session))).rejects.toBe(failure)
  ...
  it('retries a failed overlapping background write at the explicit flush barrier', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(Promise.all(barriers))).resolves.toEqual([true, true])
      expect(backend.appendAttempts).toBe(2)
      expect(backend.store.get(session.id)?.events.map(event => event.seq)).toEqual([0, 1])
    } finally {
      appendGate.resolve(true)
...
describe('PersistenceCoordinator stored identity', () => {
  it('rejects a mismatched backend header before repair or state publication', async () => {
    ...
    try {
      await expect(Promise.resolve(coordinator.load(requested))).rejects.toThrow(/stored session identity mismatch/)
  ...
  it('reserves a cold id across asynchronous storage repair', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(ctx.plugin(Object.assign((inner: Context) => {
      ...
      await expect(Promise.resolve(ctx.sessions.flush(resumed))).resolves.toBe(true)
    } finally {
      loadGate.resolve(true)
      await fiber.dispose()
      await ctx.fiber.dispose()
...
describe('PersistenceCoordinator session preparations', () => {
  ...
  it('retries invalidated prepare and load reservations', async () => {
    ...
    try {
      ...
      reserve.mockResolvedValueOnce(undefined)
      await expect(Promise.resolve(coordinator.load(loadId))).resolves.toMatchObject({ meta: { id: loadId } })
    } finally {
      await fiber.dispose()
      await ctx.fiber.dispose()
    }
  ...
  it('prefers a session that becomes live across preparation reads', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(coordinator.prepare(prepareId))).rejects.toThrow(/while it is live/)
      ...
      await expect(Promise.resolve(coordinator.inspect(inspectId))).resolves.toMatchObject({ meta: { id: inspectId } })
      ...
      await expect(Promise.resolve(coordinator.inspect(validatedInspectId)))
      ...
      await expect(Promise.resolve(coordinator.inspect(failedInspectId)))
  ...
  it('rejects a prepared commit when durable state already has a live owner', async () => {
    ...
    try {
      await expect(Promise.resolve(coordinator.prepare(id))).rejects.toThrow(/live persistence owner/)
    } finally {
      await fiber.dispose()
      await ctx.fiber.dispose()
    }
  ...
  it('observes a restored suffix initialization failure', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(live.init)).rejects.toBe(failure)
    } finally {
      preparation[Symbol.dispose]()
      await fiber.dispose()
      await ctx.fiber.dispose()
  ...
  it('writes new events after publishing a preparation with no unpublished suffix', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(ctx.sessions.flush(preparation.session))).resolves.toBe(true)
  ...
  it('retains a reserved preparation when inspection observes a newer external revision', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(coordinator.inspect(id))).resolves.toBe(cached)
      const preparations = (coordinator as unknown as {
        preparations: { reservationFor: (session: Session) => object | undefined }
      }).preparations
  ...
  it('queues a same-tick cold append behind preparation readiness', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(inspection)).resolves.toMatchObject({
        meta: { id },
        events: [...oneTurnLog(), { seq: 6 }, { seq: 7 }],
      })
      await expect(Promise.resolve(append)).resolves.toBeUndefined()
      expect(backend.loadAttempts).toBe(2)
  ...
  it('inspects an open live turn without balancing it', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(coordinator.load(session.id))).rejects.toThrow(/live turn is open/)
    } finally {
      await fiber.dispose()
      await ctx.fiber.dispose()
    }
  ...
  it('rejects preparation when storage disappears during the post-repair reload', async () => {
    ...
    try {
      await expect(Promise.resolve(coordinator.prepare(id))).rejects.toThrow(/not found/)
      expect(backend.repairAttempts).toBe(1)
      expect(backend.loadAttempts).toBe(2)
    } finally {
      await fiber.dispose()
  ...
  it('rejects append while an unpublished preparation owns the persisted cursor', async () => {
    ...
    try {
      preparation = await coordinator.prepare(id)
      await expect(Promise.resolve(coordinator.append(id, [{
        type: 'turn/start',
        seq: oneTurnLog().length,
        time: 7,
        data: { turn: 2 },
...
describe('PersistenceCoordinator observation cancellation', () => {
  it('promptly rejects a queued inspect without invoking it and keeps the same-id chain healthy', async () => {
    ...
    try {
      ...
      loadGate.resolve(true)
      await expect(Promise.resolve(prior)).resolves.toMatchObject({ meta: { id } })
      await observedAbort
      await expect(Promise.resolve(subsequent)).resolves.toMatchObject({ meta: { id } })
      expect(backend.loadAttempts).toBe(1)
      await vi.waitFor(() => {
  ...
  it('keeps a shared cold read alive when its creating inspect is cancelled', async () => {
    ...
    try {
      ...
      controller.abort(reason)
      await expect(Promise.resolve(inspection)).rejects.toBe(reason)
      loadGate.resolve(true)
  ...
  it('preserves inspect cancellation when the session concurrently becomes live', async () => {
    ...
    try {
      await expect(Promise.resolve(coordinator.inspect(id, controller.signal))).rejects.toBe(reason)
    } finally {
      get.mockRestore()
      await fiber.dispose()
      await ctx.fiber.dispose()
  ...
  it('readFrom via the seek hook: serves the suffix, maps undefined to not-found, and relays hook failures by abort state', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(coordinator.readFrom(SessionId('missing-seek'), 0))).rejects.toThrow('not found')
      ...
      await expect(Promise.resolve(coordinator.readFrom(id, 0))).rejects.toBe(hookFailure)
...
describe('PersistenceCoordinator retirement', () => {
  it('a retiring unmaterialized owner without buffered events releases its id', async () => {
    ...
    try {
      ...
      loadGate.resolve(true)
      await expect(Promise.resolve(reuseFlush)).resolves.toBe(true)
    } finally {
      loadGate.resolve(true)
      await backendFiber.dispose()
      await ctx.fiber.dispose()
  ...
  it('a replacement queued before retirement cleanup still collides with the live owner', async () => {
    ...
    try {
      ...
      appendGate.resolve(true)
      await expect(Promise.resolve(reuseFlush)).rejects.toThrow(/bound to a different live session/)
  ...
  it('a racing cold load survives retirement cleanup and rejects same-id reuse', async () => {
    ...
    try {
      ...
      loadGate.resolve(true)
      await expect(Promise.resolve(coldLoad)).resolves.toMatchObject({
        events: [{ seq: 0 }, { seq: 1 }],
      })
      ...
      await expect(Promise.resolve(ctx.sessions.flush(reuse))).rejects.toThrow(/id collision/)
      await vi.waitFor(() => {
        expect(backend.store.get(id)?.events.map(event => event.seq)).toEqual([0, 1])
      })
    } finally {
...
describe('SessionPersistence service registration', () => {
  it('provides a cancellation-aware default preparation for simple backends', async () => {
    ...
    await expect(Promise.resolve(defaultPrepare(m.id, preAborted.signal)))
    ...
    await expect(Promise.resolve(defaultPrepare(m.id, postAborted.signal)))
  ...
  it('requires SessionStore for the default preparation', async () => {
    ...
    await expect(Promise.resolve(SessionPersistence.prototype.prepare.call(persistence, id)))
  ...
  it('rejects non-JSON session metadata before registering lazy state', async () => {
    ...
    await expect(Promise.resolve(ctx.sessionPersistence.create(invalid)))
  ...
  it('rejects a legacy stored prefix during live HMR adoption', async () => {
    ...
    await expect(Promise.resolve(ctx.sessions.flush(session)))
  ...
  it('rejects a stored legacy fallback header during load', async () => {
    ...
    await expect(Promise.resolve(ctx.sessionPersistence.load(id)))

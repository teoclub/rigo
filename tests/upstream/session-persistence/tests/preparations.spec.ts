...
import { Session, SessionId } from '@teoclub/harness-session'
import { observeQueuedAbort, SessionPreparations } from '@teoclub/harness-session-persistence/src/preparations.ts'
...
describe('SessionPreparations inspection', () => {
  it('shares in-flight and ready sources, then invalidates them', async () => {
    ...
    gate.resolve(source)
    await expect(Promise.resolve(first)).resolves.toBe(source)
    await expect(Promise.resolve(second)).resolves.toBe(source)
    await expect(Promise.resolve(preparations.inspect(id, load))).resolves.toBe(source)
  ...
  it('keeps a shared load alive when its first observer cancels', async () => {
    ...
    await expect(Promise.resolve(first)).rejects.toBe(reason)
    const source = prepared(id)
    gate.resolve(source)
    await expect(Promise.resolve(joined)).resolves.toBe(source)
    await expect(Promise.resolve(preparations.inspect(id, load))).resolves.toBe(source)
    expect(load).toHaveBeenCalledOnce()
  })
...
await secondRejection
  ...
  it('removes failed and invalidated in-flight loads without changing their observers', async () => {
    ...
    await expect(Promise.resolve(preparations.inspect(failedId, () => Promise.reject(failure)))).rejects.toBe(failure)
    ...
    gate.resolve(source)
    await expect(Promise.resolve(inspection)).resolves.toBe(source)
  ...
  it('removes a load that throws before returning its promise', async () => {
    ...
    await expect(Promise.resolve(preparations.inspect(id, () => { throw failure }))).rejects.toBe(failure)
    expect(preparations.has(id)).toBe(false)
  })
...
describe('SessionPreparations reservation', () => {
  ...
  it('supports abortable reservation waits without cancelling the held reservation', async () => {
    ...
    await Promise.resolve()
    controller.abort(reason)
    await expect(Promise.resolve(waiting)).rejects.toBe(reason)
  ...
  it('removes a failed commit and wakes another waiter as invalidated', async () => {
    ...
    await expect(Promise.resolve(second)).resolves.toBeUndefined()
    expect(preparations.has(id)).toBe(false)
  })
  ...
  it('returns a post-commit cancellation to the ready pool', async () => {
    ...
    await expect(Promise.resolve(preparations.reserve(id, () => Promise.resolve(source), async (value) => {
      controller.abort(reason)
  ...
  it('does not revive an invalidated commit after post-commit cancellation', async () => {
    ...
    await expect(Promise.resolve(reservation)).rejects.toBe(reason)
    expect(preparations.has(id)).toBe(false)
  })
  ...
  it('does not reserve an entry invalidated while its commit succeeds', async () => {
    ...
    await expect(Promise.resolve(reservation)).resolves.toBeUndefined()
    expect(preparations.has(id)).toBe(false)
  })
...
describe('observeQueuedAbort', () => {
  it('relays fulfillment and rejection exactly', async () => {
    ...
    await expect(Promise.resolve(observeQueuedAbort(Promise.resolve('value'), signal))).resolves.toBe('value')
    const failure = { kind: 'failed' }
    const rejected = Promise.withResolvers<never>()
    rejected.reject(failure)
    await expect(Promise.resolve(observeQueuedAbort(rejected.promise, signal))).rejects.toBe(failure)
  })
  ...
  it('rejects promptly with an exact abort reason and ignores later settlement', async () => {
    ...
    controller.abort(reason)
    await expect(Promise.resolve(observed)).rejects.toBe(reason)
  ...
  it('observes a pre-aborted signal through the default start predicate', async () => {
    ...
    await expect(Promise.resolve(observeQueuedAbort(new Promise<never>(() => {}), controller.signal)))
  ...
  it('lets an operation that already started own cancellation settlement', async () => {
    ...
    await expect(Promise.resolve(observed)).resolves.toBe('owned')
  })
})

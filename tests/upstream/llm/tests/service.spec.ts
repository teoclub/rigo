...
import { AttachmentId } from '@teoclub/harness-attachment'
import LlmRuntime, {
  errorChain,
  GenerateOptions,
  HarnessError,
...
describe('LlmRuntime', () => {
  ...
  it('leaves middleware and consumer failures thrown', async () => {
    ...
    await expect(Promise.resolve(collect(middlewareCtx.llm.stream({
      provider: 'test',
      model: 'test',
      messages: [],
  ...
  it('discovers detached provider and advisory model metadata', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.listModels('catalog'))).resolves.toEqual([{
  ...
  it('defaults adapters to their route name and an empty advisory model list', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.listModels('plain'))).resolves.toEqual([])
    await expect(Promise.resolve(ctx.llm.listModels('missing'))).rejects.toMatchObject({ code: 'NO_ADAPTER' })
    await expect(Promise.resolve(ctx.llm.resolveModelInfo('plain', 'unlisted'))).resolves.toEqual({
      provider: 'plain', id: 'unlisted', name: 'unlisted',
    })
    await expect(Promise.resolve(ctx.llm.resolveModelInfo('missing', 'm'))).rejects.toMatchObject({ code: 'NO_ADAPTER' })
  })
  ...
  it.each([
    ...
    await expect(Promise.resolve(ctx.llm.resolveModelInfo('route', 'model')))
  ...
  it('preserves modality metadata through exact model resolution', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.resolveModelInfo('route', 'model'))).resolves.toEqual({
      provider: 'route', id: 'model', name: 'Model',
      inputModalities: ['text', 'image'],
    })
  })
  ...
  it('resolves detached model context independently of advisory catalog membership', async () => {
    ...
    expect(resolved.context).toEqual({ contextWindow: 32_000 })
    await expect(Promise.resolve(ctx.llm.resolveModelInfo('route', 'other'))).resolves.toEqual({
  ...
  it('resolves detached adapter-owned reasoning metadata and materializes its default', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.resolveCallConfig({ provider: 'route', model: 'model' }))).resolves.toEqual({
      provider: 'route',
      model: 'model',
    ...
    await expect(Promise.resolve(ctx.llm.resolveCallConfig(explicit))).resolves.toBe(explicit)
    const providerDefault = { provider: 'route', model: 'providerDefault' }
    await expect(Promise.resolve(ctx.llm.resolveCallConfig(providerDefault))).resolves.toBe(providerDefault)
  })
  ...
  it('materializes an adapter-owned maxTokens default while preserving an explicit cap', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.resolveModelInfo('route', 'model'))).resolves.toMatchObject({
  ...
  it('rejects unsupported reasoning efforts without clamping', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.resolveCallConfig({
      provider: 'route',
      model: 'model',
      reasoningEffort: ReasoningEffortId('standard'),
  ...
  it('passes cancellation through exact-model resolution', async () => {
    ...
    await expect(Promise.resolve(resolving)).rejects.toBe(reason)
  })
  ...
  it.each([
    ...
    await expect(Promise.resolve(ctx.llm.listModels('route'))).rejects.toMatchObject({ code: 'INVALID_CATALOG' })
  })
  ...
  it('LlmError extends the shared HarnessError base', async () => {
    const { HarnessError, isHarnessError } = await import('@teoclub/harness-llm')

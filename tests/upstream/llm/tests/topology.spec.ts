...
import LlmRuntime, { LlmAdapter, LlmError } from '@teoclub/harness-llm'
import type { GenerateOptions, LlmConfigurableProvider, StreamChunk } from '@teoclub/harness-llm'
...
describe('model discovery registry', () => {
  it('offers one interrogation per settings namespace and disposes with its fiber', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.discoverModels('llm-example', { baseURL: 'https://gateway.example/v1' })))
  ...
  it('refuses a namespace nothing serves and a draft with no endpoint', async () => {
    ...
    await expect(Promise.resolve(ctx.llm.discoverModels('llm-absent', { baseURL: 'https://gateway.example/v1' })))
      .rejects.toMatchObject({ code: 'NO_DISCOVERY' })
    await expect(Promise.resolve(ctx.llm.discoverModels('llm-example', { baseURL: '' })))
      .rejects.toMatchObject({ code: 'INVALID_DISCOVERY' })
    await expect(Promise.resolve(ctx.llm.discoverModels('llm-example', { provider: '', baseURL: '' })))
      .rejects.toMatchObject({ code: 'INVALID_DISCOVERY' })
    await expect(Promise.resolve(ctx.llm.discoverModels('llm-example', {})))
      .rejects.toMatchObject({ code: 'INVALID_DISCOVERY' })
    // Naming a route alone is enough: the adapter may know it without an endpoint.
    await expect(Promise.resolve(ctx.llm.discoverModels('llm-example', { provider: 'known-route' }))).resolves.toEqual([])
  })
})

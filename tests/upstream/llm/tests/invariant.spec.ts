...
import LlmRuntime, { CallId, LlmAdapter } from '@teoclub/harness-llm'
import type { GenerateOptions, StreamChunk } from '@teoclub/harness-llm'
import * as LlmInvariant from '@teoclub/harness-llm/invariant'
...
describe('LLM stream invariants', () => {
  it('accepts a complete interleaved stream grammar', async () => {
    ...
    await expect(Promise.resolve(consume(ctx, chunks))).resolves.toEqual(chunks)
  })
  ...
  it.each([
    ...
    const ctx = await setup()
    await expect(Promise.resolve(consume(ctx, chunks))).rejects.toThrow(message)
  })

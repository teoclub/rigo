...
} from '@teoclub/harness-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@teoclub/harness-protocol'
import { apply as applyClientRegistry, inject as clientRegistryInject } from '@teoclub/harness-typert-registry/client'

declare module '@teoclub/harness-protocol' {
...
describe('TypertRegistry', () => {
  ...
  it('configures an asynchronous lookup resolver independently of provider load order', async () => {
    ...
    await expect(Promise.resolve(ctx.typert.lookups.get('fixture')?.resolve('configured'))).resolves.toBe(configured)
  ...
  it('configures an asynchronous Host Context resolver independently of provider load order', async () => {
    ...
    await expect(Promise.resolve(ctx.typert.contexts.getHost('registryFixture')?.resolve('configured'))).resolves.toBe(configured)

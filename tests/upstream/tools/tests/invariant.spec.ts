...
import type { ToolExecution, ToolExecutionResult, ToolExecutionToken } from '@teoclub/harness-tools'
import * as ToolsInvariant from '@teoclub/harness-tools/invariant'
...
describe('tool-pipeline invariants', () => {
  ...
  it('rejects repeated and out-of-order pipeline stages', async () => {
    ...
    await expect(Promise.resolve(stage(ctx, 'tools/pre-execute', exec))).rejects.toThrow(/repeated/)

    const noPre = execution({ callId: CallId('call-2') })
    await expect(Promise.resolve(stage(ctx, 'tools/execute', noPre))).rejects.toThrow(/must follow tools\/pre-execute/)
  ...
  it('replays enclosed code-dispatch records on late registration', async () => {
    ...
    session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
    await ctx.plugin(InvariantRegistry)
    await expect(Promise.resolve(ctx.plugin(ToolsInvariant).then(() => undefined))).resolves.toBeUndefined()
  })
  ...
  it('rejects an unenclosed code-dispatch record on late registration', async () => {
    ...
    await ctx.plugin(InvariantRegistry)
    await expect(Promise.resolve(ctx.plugin(ToolsInvariant).then(() => undefined))).rejects.toThrow(/outside any open turn/)
  })
})

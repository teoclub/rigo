...
import { createUserMessage, CallId  } from '@teoclub/harness-llm'
import { createScope } from '@teoclub/harness-scope'
import type { Scope } from '@teoclub/harness-scope'
import SystemPrompt from '@teoclub/harness-system-prompt'
import { CodeRuntime } from '@teoclub/harness-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@teoclub/harness-code-runtime'
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@teoclub/harness-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@teoclub/harness-tools'
import type { Agent } from '@teoclub/harness-agent'
import { Session, SessionId } from '@teoclub/harness-session'
import type { JsonValue, SessionEventMap } from '@teoclub/harness-session'
...
describe('mode-aware wire contribution', () => {
  ...
  it('rejects every assembly when a non-native mode has no code runtime', async () => {
    ...
    await expect(Promise.resolve(systemPrompt.assemble())).rejects.toThrow(/requires a code runtime/)
  })
  ...
  it('rejects every assembly when the runtime language has no registered SDK renderer', async () => {
    ...
    await expect(Promise.resolve(systemPrompt.assemble())).rejects.toThrow(/no SDK renderer registered for runtime language "ruby"/)
  })
  ...
  it("rejects the assembly when toolOrder names a native tool that mode 'code' no longer contributes", async () => {
    ...
    registerEcho(ctx)
    await expect(Promise.resolve(systemPrompt.assemble())).rejects.toThrow(/toolOrder lists unregistered tool "echo"/)
  })
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@teoclub/harness-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
  ...
  it('reports the missing runtime against the agent\'s own mode', async () => {
    ...
    await expect(Promise.resolve(systemPrompt.assemble({ scope: agent })))

...
import { createUserMessage, CallId, HarnessError, type ContentBlock  } from '@teoclub/harness-llm'
import SystemPrompt from '@teoclub/harness-system-prompt'
import type { Agent } from '@teoclub/harness-agent'
import ApprovalService, { type ApprovalOutcome, type ApprovalRequest } from '@teoclub/harness-user-approval'
import ToolRuntime, {
...
describe('ToolRuntime', () => {
  ...
  it('normalizes a hostile thrown value whose inspection and coercion both throw', async () => {
    ...
    await expect(Promise.resolve(ctx.tools.execute({
      signal: testToolSignal,

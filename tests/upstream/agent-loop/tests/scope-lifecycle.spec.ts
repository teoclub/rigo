...
import { Context, symbols, type EffectMeta, type Fiber } from '@teoclub/cordis'
import LlmRuntime from '@teoclub/harness-llm'
import SessionStore, { SessionId, type SessionEvent } from '@teoclub/harness-session'
import SystemPrompt from '@teoclub/harness-system-prompt'
import ToolRuntime, { defineContentToolFixture } from '@teoclub/harness-tools'
import AgentRegistry, { agentEvents, assembleContextFor } from '@teoclub/harness-agent'

import type { Agent } from '@teoclub/harness-agent'
import { scopeOf } from '@teoclub/harness-scope'
...
describe('agent scope lifecycle', () => {
  it('rejects an already-aborted creation signal before publishing either object', async () => {
    ...
    await expect(Promise.resolve(ctx.agents.create({
  ...
  it('uses signal only for creation: aborts pending setup but not a returned live handle', async () => {
    ...
    await expect(Promise.resolve(pending)).rejects.toThrow('cancel pending creation')
  ...
  it('owner unload aborts a pending setup and publishes nothing', async () => {
    ...
    await owner.dispose()
    await expect(Promise.resolve(creating)).rejects.toThrow(/owner disposed during setup/)
    ...
    await expect(Promise.resolve(creating2)).rejects.toThrow(/owner disposed during setup/)
    await unload2
  ...
  it('an AgentLoop unload aborts pending setup, awaits cleanup, and releases both ids', async () => {
    ...
    await expect(Promise.resolve(creating)).rejects.toThrow(/agent loop is not active/)
    expect(published).toEqual([])
  ...
  it('AgentLoop unload is a structural co-owner of every live programmatic agent', async () => {
    ...
    await expect(Promise.resolve(loop.createAgent(ctx, {

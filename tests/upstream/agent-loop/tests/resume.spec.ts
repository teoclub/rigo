...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId, SessionPreparation } from '@teoclub/harness-session'
...
import AgentRegistry, { type Agent } from '@teoclub/harness-agent'

import MemoryPersistence from '../../../support/memory-persistence.ts'
...
describe('the session-persistence Agent Note: AgentLoop factory create/resume', () => {
  ...
  it('normalizes a non-Error resume publication failure for rollback and rethrows it', async () => {
    ...
    await expect(Promise.resolve(ctx.agents.resume({
      resumeSessionId: sessionId,
  ...
  it('createAgent rejects a duplicate identity without orphaning a session', async () => {
    ...
    await expect(Promise.resolve(ctx.agents.create({ sessionId }))).rejects.toThrow(/already exists/)
  ...
  it('resume cannot crash-repair a turn owned by a live agent', async () => {
    ...
    await expect(Promise.resolve(ctx.agents.resume({ resumeSessionId: sessionId })))
  ...
  it('resume of a session with no cwd carries an undefined cwd header', async () => {
    ...
    await ctx2.plugin(MemoryPersistence, { root })
  ...
  it('owner unload aborts resume setup and cannot publish after the callback settles', async () => {
    ...
    await owner.dispose()
    await expect(Promise.resolve(resuming)).rejects.toThrow(/owner disposed during setup/)
    expect(published).toEqual([])
    expect(ctx.agents.get(sessionId)).toBeUndefined()
    expect(ctx.sessions.get(sessionId)).toBeUndefined()
  ...
  it('owner unload aborts a never-settling persistence preparation, releases the identity, and blocks late publication', async () => {
    ...
    const rejection = promptly(resuming).then(() => { throw new Error('expected resume rejection') }, (error: unknown) => expect(error instanceof Error ? error.message : String(error)).toMatch(/owner disposed during setup/))
  ...
  it('AgentLoop unload aborts persistence preparation and awaits wrapper settlement', async () => {
    ...
    await preparationStarted.promise
    const rejection = promptly(resuming).then(() => { throw new Error('expected resume rejection') }, (error: unknown) => expect(error instanceof Error ? error.message : String(error)).toMatch(/agent loop is not active/))
  ...
  it('resume rejects when session persistence is not configured', async () => {
    ...
    await ctx.plugin(AgentLoop, { agents: [] })
    ctx.llm.registerAdapter(['mock'], adapter)
    await expect(Promise.resolve(ctx.agents.resume({ resumeSessionId: SessionId('nope') })))
...
describe('creation and resume cancellation edges', () => {
  it('rejects create() with a pre-aborted signal, including a non-Error reason', async () => {
    ...
    await expect(Promise.resolve(promptly(ctx.agents.create({
  ...
  it('a non-Error abort reason arriving during setup is wrapped for the caller', async () => {
    ...
    await expect(Promise.resolve(promptly(creating))).rejects.toThrow(/creation aborted/)
  ...
  it('rejects when setup synchronously aborts its caller signal', async () => {
    ...
    await expect(Promise.resolve(promptly(creating))).rejects.toThrow('setup synchronously cancelled')
  ...
  it('resume with a pre-aborted caller signal rejects out of the load race', async () => {
    ...
    await expect(Promise.resolve(promptly(ctx.agents.resume({
      resumeSessionId: sessionId,
      agentOptions: { provider: 'mock', model: 'mock' },
      signal: controller.signal,
  ...
  it('factory teardown during a hung resume preparation rejects with loop-inactive', async () => {
    ...
    const rejection = promptly(resuming).then(() => { throw new Error('expected resume rejection') }, () => undefined)
...
describe('configured-start failure edges', () => {
  it('a non-Error mid-prepare abort reason is wrapped for the resume caller', async () => {
    ...
    await expect(Promise.resolve(promptly(resuming))).rejects.toThrow(/creation aborted/)
    expect(ctx.agents.get(sessionId)).toBeUndefined()
    await ctx.fiber.dispose()
  })
  ...
  it('a failing exact-id restore over an existing artifact stays loud', async () => {
    ...
    await configured.plugin(MemoryPersistence, { root })

...
import { createUserMessage, CallId, StreamChunk  } from '@teoclub/harness-llm'
import SessionStore, { SessionEvent, SessionId } from '@teoclub/harness-session'
import SystemPrompt from '@teoclub/harness-system-prompt'
import LlmRuntime from '@teoclub/harness-llm'
import ToolRuntime, { defineContentToolFixture, TOOL_ABORTED_BEFORE_DISPATCH, TOOL_RUNTIME_SCHEDULER, type PostToolDecision, type PreToolDecision } from '@teoclub/harness-tools'
import AgentRegistry, { type Agent } from '@teoclub/harness-agent'
import AgentLoop, { DEFAULT_MAX_PARALLEL_TOOL_CALLS } from '@teoclub/harness-agent-loop'
...
describe('tool-call scheduler: rolling pool honors maxParallelToolCalls', () => {
  it('rejects invalid global maxParallelToolCalls config at plugin load', async () => {
    await expect(Promise.resolve(harness(new MockAdapter([]), 0))).rejects.toThrow()
    await expect(Promise.resolve(harness(new MockAdapter([]), 1.5))).rejects.toThrow()
  })

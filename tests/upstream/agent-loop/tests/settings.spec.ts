...
import { SettingsProvider } from '@teoclub/harness-settings'
import type { SettingsNamespace } from '@teoclub/harness-settings'
import AgentLoop, { AGENT_LOOP_SETTINGS_NAMESPACE } from '@teoclub/harness-agent-loop'
...
describe('agent-loop settings section', () => {
  ...
  it('refuses a non-positive cap at the write', async () => {
    ...
    await expect(Promise.resolve(bench.ctx.settings.update(AGENT_LOOP_SETTINGS_NAMESPACE, { maxParallelToolCalls: 0 })))
      .rejects.toThrow()

...
import SessionStore, { SessionId, SessionPreparation } from '@teoclub/harness-session'
...
import AgentLoop, { CONFIGURED_AGENT_IDENTITIES_KEY } from '@teoclub/harness-agent-loop'
...
describe('config-driven session id', () => {
  ...
  it('rejects an empty exact id before publishing an agent', async () => {
    const ctx = await makeCoreContext()
    await expect(Promise.resolve(ctx.plugin(AgentLoop, {
  ...
  it('accepts one exact fresh id and rejects it alongside a resume id', async () => {
    ...
    await expect(Promise.resolve(conflicting.plugin(AgentLoop, {
      agents: [{
        id: 'main',
  ...
  it('config-driven create uses a fresh ${id}-session-<uuid> per run (restart-safe)', async () => {
    ...
    await ctx1.plugin(MemoryPersistence, { root })

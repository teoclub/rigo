...
import LlmRuntime, { createUserMessage, CallId, LlmError, StreamChunk  } from '@teoclub/harness-llm'
...
describe('agent loop', () => {
  ...
  it('cancels queued wakeup work together with an active maintenance task', async () => {
    ...
    await expect(Promise.resolve(maintenance)).rejects.toThrow('maintenance aborted')

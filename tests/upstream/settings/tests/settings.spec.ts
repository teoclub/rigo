...
import { SettingsProvider, SettingsConflictError, deepEqualJson, installSettingsSection, settingsNamespace, type SettingsNamespace, type SettingsScope, type SettingsUpdateSource } from '@teoclub/harness-settings'
...
describe('provider metadata', () => {
  it('does not advertise a local document unless the provider overrides it', async () => {
    ...
    await expect(Promise.resolve(ctx.settings.prepareDocument())).resolves.toBeUndefined()
  })
})
...
describe('registration', () => {
  ...
  it('refuses a write its owner could not act on, and keeps the last good value for a stored one', async () => {
    ...
    await expect(Promise.resolve(ctx.settings.update(ns, { fontSize: 4 }))).rejects.toThrow(/unreadable/)
...
describe('update', () => {
  ...
  it('rejects an invalid patch before persisting anything', async () => {
    ...
    const events = recordUpdates(ctx)
    const scope = ctx.settings.register(settingsNamespace('ui-theme'), ThemeSchema)
    await expect(Promise.resolve(scope.update({ fontSize: 'big' }))).rejects.toThrow()
    expect(provider.persisted).toEqual([])
    expect(events).toEqual([])
  ...
  it('rejects a non-object patch', async () => {
    const { ctx } = await boot()
    const scope = ctx.settings.register(settingsNamespace('ui-theme'), ThemeSchema)
    await expect(Promise.resolve(scope.update([1]))).rejects.toThrow(TypeError)
    await expect(Promise.resolve(scope.update(new Date() as unknown as object))).rejects.toThrow(TypeError)
    await expect(Promise.resolve(scope.replace([1]))).rejects.toThrow(/replace for "ui-theme"/)
  })
  ...
  it('rejects an unregistered namespace', async () => {
    const { ctx } = await boot()
    await expect(Promise.resolve(ctx.settings.update(settingsNamespace('missing'), {})))
  ...
  it('rejects on a read-only provider before reaching persist', async () => {
    ...
    const scope = ctx.settings.register(settingsNamespace('ui-theme'), ThemeSchema)
    await expect(Promise.resolve(scope.update({ theme: 'light' }))).rejects.toThrow(/read-only/)
    expect(provider.persisted).toEqual([])
  })
})
...
describe('second review regressions', () => {
  ...
  it('rejects an update queued after the registrant fiber disposed', async () => {
    ...
    await fiber
    await fiber.dispose()
    await expect(Promise.resolve(scope!.update({ theme: 'light' }))).rejects.toThrow(/disposed|not registered/)
  })
  ...
  it('drains in-flight writes at service dispose and rejects later ones', async () => {
    ...
    await expect(Promise.resolve(service.update(settingsNamespace('ui-theme'), { theme: 'dark' })))
  ...
  it('rejects a function value as not JSON-compatible', async () => {
    const { ctx } = await boot()
    const scope = ctx.settings.register(settingsNamespace('ui-theme'), ThemeSchema)
    await expect(Promise.resolve(scope.update({ theme: () => 'dark' })))
  ...
  it('rejects a write still queued when the service disposes', async () => {
    ...
    await fiber.dispose()
    await first
    await expect(Promise.resolve(second)).rejects.toThrow(/disposed before the queued/)
  })
  ...
  it('rejects a write still queued when the registrant disposes', async () => {
    ...
    await fiber.dispose()
    await first
    await expect(Promise.resolve(second)).rejects.toThrow(/registration was disposed before the queued/)
  })
...
describe('third review regressions', () => {
  ...
  it('rejects a Date at its path before anything persists', async () => {
    ...
    await expect(Promise.resolve(scope.update({ value: { at: new Date(0) } })))
  ...
  it.each([
    ...
    const { ctx } = await boot()
    const scope = ctx.settings.register(settingsNamespace('ui-theme'), z.object({ value: z.any() }))
    await expect(Promise.resolve(scope.update(patch))).rejects.toThrow(message)
  })
  ...
  it('rejects a circular patch instead of storing an alias-looped document', async () => {
    ...
    cyclic['self'] = cyclic
    await expect(Promise.resolve(scope.update({ value: cyclic }))).rejects.toThrow(/circular reference at \$\.value\.self/)
    const loop: unknown[] = []
    loop.push(loop)
    await expect(Promise.resolve(scope.update({ value: loop }))).rejects.toThrow(/circular reference at \$\.value\[0\]/)
  })
...
describe('mutate (path-addressed writes)', () => {
  ...
  it('refuses a non-object at the section root, leaving the stored section alone', async () => {
    const ctx = await mounted({ keyed: { apiKey: 'sk-stored' } })
    await expect(Promise.resolve(ctx.settings.mutate(KEYED, [{ op: 'set', path: [], value: 'a whole section' }])))
  ...
  it('rejects ops that are not an array at all', async () => {
    const ctx = await mounted({ keyed: { apiKey: 'sk-stored' } })
    await expect(Promise.resolve(ctx.settings.mutate(KEYED, { op: 'unset', path: ['apiKey'] } as never)))
  ...
  it('rejects a malformed op before anything is queued', async () => {
    const ctx = await mounted({ keyed: { apiKey: 'sk-stored' } })
    await expect(Promise.resolve(ctx.settings.mutate(KEYED, [{ op: 'delete' } as never])))
      .rejects.toThrow(/must be \{op:'set'\|'unset', path\}/)
    await expect(Promise.resolve(ctx.settings.mutate(KEYED, [{ op: 'unset', path: ['a', 1] as never }])))
  ...
  it('rejects a value that lossless JSON cannot represent', async () => {
    ...
    await expect(Promise.resolve(ctx.settings.mutate(KEYED, [{ op: 'set', path: ['baseURL'], value: new Date() }])))
...
describe('revision and conflict detection', () => {
  ...
  it('refuses a write whose expected revision is stale, leaving the winner in place', async () => {
    ...
    await expect(Promise.resolve(ctx.settings.update(REV, { a: 'from-tab-A' }, opened)))
  ...
  it('moves the revision for an external edit the provider publishes', async () => {
    ...
    await expect(Promise.resolve(ctx.settings.update(REV, { b: 'stale' }, 0))).rejects.toThrow(SettingsConflictError)
  })

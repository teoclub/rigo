...
describe('idleWatchdog', () => {
  ...
  it('arms only while next is outstanding and rearms the same signal for later demand', async () => {
    ...
    await expect(Promise.resolve(firstNext)).resolves.toEqual({ done: false, value: 1 })
    ...
    await expect(Promise.resolve(secondNext)).rejects.toBe(stableSignal.reason)
  })
  ...
  it('rearms outstanding demand on an out-of-band activity pulse', async () => {
    ...
    await expect(Promise.resolve(next)).rejects.toBe(watchdog.signal.reason)
  ...
  it('clears an outstanding arm on disposal', async () => {
    ...
    await expect(Promise.resolve(watchdog.next({ next: () => Promise.resolve({ done: true, value: undefined }) })))
  ...
  it('rejects invalid bounds and concurrent iterator demand', async () => {
    ...
    await expect(Promise.resolve(watchdog.next(iterator))).rejects.toThrow(/already outstanding/)
    pending.resolve({ done: true, value: undefined })
  })
})

...
import {
  ...
  expandHomePath,
  resolveDshHome,
} from '@teoclub/harness-home-paths'
...
describe('dsh path helpers', () => {
  ...
  it('canonicalizes a watcher ancestor while preserving a missing suffix', async () => {
    ...
    try {
      ...
      await expect(Promise.resolve(canonicalizeWatchPath(join(alias, 'later', 'config.yml')))).resolves.toBe(
        join(await realpath(target), 'later', 'config.yml'),
      )
      const file = join(root, 'file')
      await writeFile(file, 'not a directory')
      await expect(Promise.resolve(canonicalizeWatchPath(join(file, 'child')))).rejects.toMatchObject({ code: 'ENOTDIR' })
    } finally {

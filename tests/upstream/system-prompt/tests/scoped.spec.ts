...
import SystemPrompt, { TOOL_ORDER_REST, renderContextSnapshot, renderPrompt } from '@teoclub/harness-system-prompt'
import type { Config, PromptAssembly } from '@teoclub/harness-system-prompt'
...
describe('scoped tool providers and toolOrder × restriction', () => {
  ...
  it('a toolOrder entry restricted away for a scope is a normal absence, while a typo still throws', async () => {
    ...
    await expect(Promise.resolve(bad.systemPrompt.assemble())).rejects.toThrow('toolOrder lists unregistered tool "basj"; known tools: bash, read')
  })
})

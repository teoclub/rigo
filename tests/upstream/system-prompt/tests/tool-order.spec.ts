...
import SystemPrompt, { PromptAssembly, TOOL_ORDER_REST } from '@teoclub/harness-system-prompt'
import type { ToolSchema } from '@teoclub/harness-llm'
...
describe('SystemPrompt tool order', () => {
  ...
  it('rejects the assembly when toolOrder names a tool that is not registered (misconfiguration blocks work)', async () => {
    ...
    await expect(Promise.resolve(ctx.systemPrompt.assemble())).rejects.toThrow(
  ...
  it.each([
    ...
    await expect(Promise.resolve(new Context().plugin(SystemPrompt, { toolOrder }))).rejects.toThrow(`must contain the "${TOOL_ORDER_REST}" rest entry`)
  })
  ...
  it.each([
    ...
    await expect(Promise.resolve(new Context().plugin(SystemPrompt, { toolOrder }))).rejects.toThrow('more than once')
  })

...
import * as SystemPromptInvariant from '@teoclub/harness-system-prompt/invariant'
...
describe('system-prompt invariants', () => {
  it('accepts a well-formed authoritative assembly', async () => {
    ...
    await expect(Promise.resolve(assemble(ctx, valid()))).resolves.toEqual(valid())
  })
  ...
  it.each([
    ...
    const ctx = await setup()
    await expect(Promise.resolve(assemble(ctx, assembly))).rejects.toThrow(message)
  })
})

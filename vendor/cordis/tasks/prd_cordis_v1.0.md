...
>
> 上游回滚 PR #932 后，Loader 条目更新不再具备回滚能力。本需求正文仅作为
> 设计历史保留；当前语义以 `spec_cordis_v1.0.md` §5.1.4 的作废说明为准，
> 决策记录见 `docs/upstream.md` 的 “Re-sync record”。仍然成立的只有：
> 无法读取/解析的配置编辑保留运行中的树，且失败的配置不会写回文件。

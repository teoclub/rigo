import { defineProperty } from '@teoclub/kit'
...
export class RegistryService {
   ...
   *
   * Disposal is **initiated**, not awaited: this method returns once the
   * runtime record is removed, while the fibers' disposers may still be
   * running. Await each fiber's `dispose()` if you need completion; an
   * awaiting variant is deferred to a later release.
   *
   * @param plugin — any supported plugin shape.

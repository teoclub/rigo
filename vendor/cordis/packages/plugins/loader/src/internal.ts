 ...
 * - Parameter order for resolveSync flipped in prereleases and was reverted:
*   early 24.x prereleases shipped resolveSync(parentURL, request), but released
*   Node 24 (verified on 24.11) uses the classic resolveSync(specifier,
*   parentURL, importAttributes). Consumers must feature-detect the accepted
*   shape (see the HMR plugin's _resolveSyncShape probe) instead of trusting
*   `version` alone.
